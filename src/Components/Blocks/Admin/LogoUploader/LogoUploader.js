...
class LogoUploader extends PureComponent {
  ...
  handleChange = async e => {
    ...
    if (file) {
      ...
      const { ok, payload } = await onUploadFile('/admin/banners', file)

      if (ok) {
        this.setState({ isLoading: false })
        const imageId = head(keys(get(payload, 'data.banners', {})))
...
LogoUploader.defaultProps = { image: null, title: '' }
